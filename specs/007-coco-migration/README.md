---
title: "Coco - Universal Local AI Gateway (Migration from Claudio)"
status: complete
created: "2026-03-10"
---

# Coco - Universal Local AI Gateway (Migration from Claudio)

This spec is organized into sub-spec files.

## Sub-Specs

- **[SPECIFICATION](./SPECIFICATION.md)** - Feature specification
- **[RESEARCH](./RESEARCH.md)** - Research notes
- **[DATA_MODEL](./DATA_MODEL.md)** - Data model
- **[PLAN](./PLAN.md)** - Implementation plan
- **[QUICKSTART](./QUICKSTART.md)** - Quickstart guide
- **[TASKS](./TASKS.md)** - Task list
- **[CONTRACTS](./CONTRACTS.md)** - Contracts
